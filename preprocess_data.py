...
def preprocess_data(src, dst, 
    ...
    directs = [(os.path.join(src, folder), os.path.join(dst, folder), 
               src_meta) for folder in sorted(folders)]
    # _preprocess_data(directs[0])
    _=pool.map(_preprocess_data, directs)
...
def _preprocess_data(src_dst_meta):
    ...
    # metaData = pd.read_csv(open(src_meta))
    ...
    for filepath in sorted(glob.glob(os.path.join(src, "*.wav"))):
        ...
        embedded_data = np.squeeze(embedded_data)
...
def create_splits(src, dst):
    ...
    player_split = list(torch.randperm(10).numpy())

    print("creating splits")
    splits={"player":{   "train":[player_split[:6]],   # currently only considered 1 split

                        "validate":[player_split[6:8]],
            ...
            for task in ["player"]:
                with open(os.path.join(dst, "AlexNet_{}_{}_{}.txt".format(task, split, modus)), mode = "w") as txt_file:
                    for vp in splits[task][modus][split]:
                        for filepath in glob.glob(os.path.join(src, "{:d}".format(vp), "AlexNet*.hdf5")):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', '-src', default=os.path.join(os.getcwd(), "CBFdataset-iso-PMTs/audio"), help="Path to folder containing each participant's data directory.")
    ...
    parser.add_argument('--meta', '-m', default=os.path.join(os.getcwd(), "CBFdataset-iso-PMTs/annotation",
                                                             "CBFdataset_iso_metadata.csv"), help="Path to meta_information json file.")
